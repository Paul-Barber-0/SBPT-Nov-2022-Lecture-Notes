...
console.log(`The United States has had ${presidents.length} presidents.`);
...
livingPresidents = livingPresidents.map(president => president.first + ' ' + president.last);
...
let deadPresidents = presidents.filter(president => president.passed != undefined);
deadPresidents.forEach(president => {
    console.log(`${president.first} ${president.last} died at ${president.passed - president.year} years old.`)
});