...
router.get('/:id', (req, res) => {
    ...
    /* 
        - used to help us locate one item in our datebase
        - ID typically has a unique value to help us find that ONE item
        - Can help us search items.
        */
    try {
        let { id } =  req.params;
...
/* 
    - Make a GET route
        - no endpoint is requied to access.
...
router.post('/', (req, res) => {
    try {
        ...
        fs.readFile('./helpers/db.json', (err, data) => {
            // 1. The path needs to be relative to where fs is located (node_modules), NOT from the controllers location.
            if (err) throw err; // conditional to deal with any error it comes across.

            const db = JSON.parse(data); // Parses the JSON file and provides an array of our objects back so we can manipulate them.
            ...
            /* 
                - Requires
                    ...
                    - Callback functioin to handle errors.
            */
