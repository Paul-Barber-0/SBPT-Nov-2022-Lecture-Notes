...
*/
        .then(response => response.json()) // "json-ifyng our string"
        // .then(data => log(data.title)); //looking at "title" within our JSON object
        .then(data => log(data));
...
The Process:
    ...
    3. use a .json() on response to finsh reading entire ReadableStream.
    4. Chained another .then() to catch and read response object and ..
    ...
    Fetch differs from jQuery:
        - The promise returned won't reject on HTTP error status even if its 404 or 500 (internal / external server error)
*/

const url = 'https://meowfacts.herokuapp.com/';

function getCatFact2() {
    fetch(url)
    .then(r => r.json())
    .then(d => log(d.data[0]));
}
