...
router.post('/:vehicle', async (req, res) => {
    try {
        ...
        const vehicleCheck = await Vehicle.find({__id: vehicle});
        if(!vehicleCheck) throw new Error('Vehicle not available');

        //4. Build task object that will be put into the collection.
            //a. What is a user doesn't provide a date? How can we assign it?
        ...
        await Vehicle.findOneAndUpdate({_id: vehicle}, {$push: {tasks: newVehicle}});
